...
void parkTheBus() {
    if (y > FIELD_HEIGHT / 3 * 2) {
        goToTarget(FIELD_WIDTH / 2, FIELD_HEIGHT - BOUNDARY - 100, MAX_RADAR_ERROR, 100, 250);
    } else {
        if (x > FIELD_WIDTH / 3 * 2) {
            goToTarget(FIELD_WIDTH - 500, FIELD_HEIGHT / 3 * 2 + 300, MAX_RADAR_ERROR, MAX_MOTOR);
        } else {
            goToTarget(500, FIELD_HEIGHT / 3 * 2 + 300, MAX_RADAR_ERROR, MAX_MOTOR);
        }
    }
}
